...
def getPrecoFruta(fruta):
    for k,v in estoque.items():
        if fruta == k:
            print(f"a {fruta} custa R$ {v['preco']}")
            return v['preco'] 
...
while ligado:
    ...
    if opcao_int == 3:
        ...
        for k,v in estoque.items():
            if produtoVenda == k:
                print(f"Quantidade atual de {k}: {v['quantidade']}")
                quantidadeVenda = int(input(f"qual a quantidade de {produtoVenda} que voce quer vender?\n"))
                if   quantidadeVenda > v["quantidade"]:
                    print("Nao temos estoque")
                    continue
                else:
    ...
    if opcao_int == 4:
        print("Qual fruta voce quer atualizar o preco?")
        for k,v in estoque.items():
            print(k)
        
        frutaUpdatePreco = input("")
        for k,v in estoque.items():
            if frutaUpdatePreco == k:
                print(f"Preco atual: {v['preco']}")
                novoPreco = float(input(f"Qual o novo preco da fruta {k}? "))
                v['preco'] = novoPreco
            else:
                continue
